...
/*			功能： 得到两个日期相差的天数（每一个月按30天，一年360天计算）
			...
			/*功能： 校验一个限制小数位数的大于等于0的浮点数
			入口参数：	
						CheckCtl: 要校验的输入框
						disptext: 出错显示的信息	
						floatcount: 小数的最高位数（如果没有该参数，则默认为2位）
						如果没有page参数，有floatcunt参数则：把page 置null
						如:	CheckFN(CheckCtl,"出错显示的信息",null,5)				add by aiai*/
			function CheckFNDigit(CheckCtl,disptext,page,floatcount)
			{					
				var s=new String(Trim(CheckCtl.value));
				temp=parseFloat(s);
				var result=true;
			...
			入口参数：	
						...
						Msg: 出错显示的信息  
						IsMaxClose: 上限是否是闭区间
						IsMinClose: 下限是否是闭区间  add by aiai*/
						
			function CheckFloatRange1(CheckCtl,Min, Max,Msg,page, IsMaxClose, IsMinClose)
			{			
				if (!IsNum(CheckCtl,Msg,page,null,1))
					return false;
				v=parseFloat(Trim(CheckCtl.value));	
